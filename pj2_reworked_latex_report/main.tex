...
The VGG-A experiments follow the provided \texttt{VGG\_Loss\_Landscape.py}
setting more closely than the first-part CNN experiments. I use the standard
CIFAR-10 train/test split, batch size 128, 20 epochs, cross entropy, Adam, and
the simple transform \texttt{ToTensor + Normalize(0.5, 0.5)} without the stronger
data augmentation used in Section 1. Four learning rates are tested to satisfy
the loss-landscape requirement.
...
VGG-A & 0.002 & 9,750,922 & 10.00\% \\
VGG-A & 0.0001 & 9,750,922 & 76.83\% \\
VGG-A & 0.0005 & 9,750,922 & 79.36\% \\
VGG-A-BN & 0.001 & 9,756,426 & \textbf{82.17\%} \\
VGG-A-BN & 0.002 & 9,756,426 & 81.19\% \\
VGG-A-BN & 0.0001 & 9,756,426 & 75.03\% \\
VGG-A-BN & 0.0005 & 9,756,426 & 81.76\% \\
\bottomrule
\end{tabular}
\end{table}
...
The best VGG-A-BN accuracy is 82.17\%, while the best VGG-A accuracy is 79.36\%.
BN improves the best test accuracy by 2.81 percentage points. It is also more
stable under a large learning rate: when the learning rate is 0.002, vanilla
VGG-A collapses to around chance accuracy, while VGG-A-BN still reaches 81.19\%.
...
between them using \texttt{matplotlib.pyplot.fill\_between}. This reproduces the
same style of loss-landscape visualization as the provided sample code.
...
\label{fig:vgg-loss-landscape}
\end{figure}

The BN model shows a lower loss band and smoother convergence across learning
rates. This visualization supports the interpretation that BN smooths the
optimization process and reduces sensitivity to the choice of step size.
...
accuracy from 79.36\% to 82.17\% under the sample-code-style VGG setting and
produces a smoother loss landscape. These results satisfy the project
requirements on CIFAR-10 training, optimization comparison, BN comparison, and
